...
import {
  AlertTriangle, ArrowUpRight, BarChart3, Briefcase,
  CheckCircle2, ChevronRight, Copy, Download, FileText,
  FolderOpen, Loader2, Paperclip, RefreshCw, TrendingUp, XCircle,
} from "lucide-react";
import { Transaction } from "@/types/transaction.types";
import { Evidence } from "@/types/evidence.types";
import { exportDashboardCSV } from "@/utils/export";
import {
  computeTransactionMetrics, enrichTransactions, findDuplicateIds,
  groupEvidenceByMonth, groupTransactionsByMonth, formatMonthLabel,
...
const RESPONSIVE = `
  ...
  @media (max-width: 1024px) {
    ...
    .db-three-col    { grid-template-columns: repeat(2, 1fr); }
  }

  @media (max-width: 768px) {
    .db-shell        { padding: 16px 16px 32px; gap: 16px; }
    .db-metrics-row  { grid-template-columns: repeat(2, 1fr); gap: 12px; }
    .db-three-col    { grid-template-columns: 1fr; }
    .db-two-col      { grid-template-columns: 1fr; }
  }

  @media (max-width: 480px) {
    .db-shell        { padding: 12px 12px 24px; gap: 12px; }
    .db-metrics-row  { grid-template-columns: 1fr; gap: 10px; }
    .db-metrics-single { gap: 10px; }
  }
`;

/* ── Sub-components ────────────────────────────────────────── */
function DashboardHeader({
  userName, userEmail, orgName, roleLabel, onExport,
}: {
  userName: string; userEmail: string; orgName?: string;
  roleLabel: string; onExport: () => void;
}) {
    ...
    <div style={s.header}>
      <div>
        ...
        <p style={s.headerMeta}>
          {roleLabel}{orgName ? ` · ${orgName}` : ""}{" · "}{userEmail}
        </p>
...
function MetricCard({
  icon, value, label, trend, color, onClick,
}: {
  ...
  return (
    <div
      style={{
        ...
        boxShadow: hovered ? "0 4px 16px rgba(15,23,42,0.06)" : "none",
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: maxHeight, padding: "8px 0" }}>
        {data.map((d, i) => (
          <div key={d.label} title={`${d.label}: ${d.count}`} style={{
            flex: 1, background: i === data.length - 1 ? CHART_BAR_ACTIVE : CHART_BAR,
            borderRadius: "4px 4px 0 0",
            height: `${Math.max((d.count / max) * 100, 8)}%`, minHeight: 6,
          }} />
        ))}
      </div>
...
export function TransactionIntegrityDashboard({ transactions, evidence, user, roleLabel }: Props) {
  ...
  const handleExport = () => {
    exportDashboardCSV({
      role: roleLabel, userName: user?.fullName ?? "", orgName: user?.organisationName,
      metrics: [
      ...
      tables: [
        { title: "Transactions Missing Evidence", headers: ["ID","Name","Counterparty","Amount","Date"], rows: missingTxns.map((t) => [t.id, t.name, t.counterparty, t.amount, t.date]) },
        { title: "Duplicate Transactions",        headers: ["ID","Name","Counterparty","Amount","Date"], rows: duplicateTxns.map((t) => [t.id, t.name, t.counterparty, t.amount, t.date]) },
      ],
    });
  };
    ...
    <div style={s.pageBg}>
      ...
      <div className="db-shell">
        ...
        <div className="db-metrics-row">
          <MetricCard icon={<XCircle size={18} />}    value={metrics.noEvidence}            label="Transactions with No Evidence" trend={`${metrics.pending} pending`} color="#dc2626" onClick={() => router.push("/transactions")} />
          <MetricCard icon={<Copy size={18} />}        value={metrics.duplicateTransactions}  label="Duplicate Transactions"                                              color="#d97706" onClick={() => router.push("/transactions")} />
          <MetricCard icon={<Briefcase size={18} />}   value={metrics.totalTransactions}      label="Total Transactions"                                                  color="#475569" onClick={() => router.push("/transactions")} />
          <MetricCard icon={<FolderOpen size={18} />}  value={metrics.totalEvidence}          label="Total Evidence Files"                                                color="#475569" onClick={() => router.push("/evidence")}     />
        </div>

        {/* 1 wide metric */}
        <div className="db-metrics-single">
          <MetricCard icon={<BarChart3 size={18} />} value={`${metrics.completedPercent}%`} label="Completed Transactions" trend={`${metrics.completed} of ${metrics.totalTransactions}`} color="#15803d" onClick={() => router.push("/transactions")} />
        </div>
          ...
          <CardShell title="Missing Evidence" count={`${missingTxns.length} transactions`} onRefresh={handleRefresh}>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {missingTxns.slice(0, 5).map((t) => (
                <ListItem key={t.id} icon={<AlertTriangle size={16} />} iconBg="#fef2f2" iconColor="#dc2626" title={t.name} subtitle={`${t.counterparty} · RWF ${t.amount.toLocaleString()}`} rightLabel="Pending" rightColor="#d97706" onClick={() => router.push(`/transactions?transactionId=${t.id}`)} />
              ))}
              {missingTxns.length === 0 && <EmptyState icon={<CheckCircle2 size={24} style={{ color: "#16a34a" }} />} message="All transactions have evidence" />}
            </div>
          </CardShell>
          ...
          <CardShell title="Duplicate Transactions" count={`${duplicateTxns.length} flagged`} onRefresh={handleRefresh}>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {duplicateTxns.slice(0, 5).map((t) => (
                <ListItem key={t.id} icon={<Copy size={16} />} iconBg="#fffbeb" iconColor="#d97706" title={t.name} subtitle={`${t.counterparty} · RWF ${t.amount.toLocaleString()}`} rightLabel="Duplicate" rightColor="#d97706" onClick={() => router.push(`/transactions?transactionId=${t.id}`)} />
              ))}
              {duplicateTxns.length === 0 && <EmptyState icon={<CheckCircle2 size={24} style={{ color: "#16a34a" }} />} message="No duplicate transactions" />}
            </div>
          </CardShell>
        ...
        <div className="db-two-col">
          <CardShell title="Transaction Status" onRefresh={handleRefresh}>
            ...
            <SegmentedBar segments={[{ value: metrics.completed, color: "#16a34a" }, { value: metrics.pending, color: "#94a3b8" }]} />
        ...
        {/* 2-column: evidence charts */}
        <div className="db-two-col">
          <CardShell title="Evidence Attachments by Month" onRefresh={handleRefresh}>
            {evByMonth.length > 0 ? <BarChart data={evByMonth} /> : <EmptyState icon={<Paperclip size={24} />} message="No evidence uploads yet" />}
          </CardShell>

          <CardShell title="Evidence vs Missing" onRefresh={handleRefresh}>
            <SegmentedBar segments={[{ value: metrics.totalEvidence, color: "#16a34a" }, { value: metrics.noEvidence, color: "#dc2626" }]} />
...
/* ── Static styles (non-grid) ──────────────────────────────── */
const s: Record<string, React.CSSProperties> = {
  pageBg: { minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Inter', system-ui, -apple-system, sans-serif" },
  header: {
  ...
  listItem: {
    ...
    padding: "10px 12px", borderRadius: 10, gap: 12, transition: "background 0.15s",
  },
  iconBtn: {
